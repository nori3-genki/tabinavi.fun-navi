 ...
 * 
 * @package HRS
 * @version 4.4.0 ✅ HQCスコア名変更・個別スコア対応
 * 変更点:
 * - _hrs_hqc_h_score → _hrs_h_score
 * - _hrs_hqc_q_score → _hrs_q_score  
 * - _hrs_hqc_c_score → _hrs_c_score
 * - HQC総合スコア自動計算（H×0.4 + Q×0.4 + C×0.2）
 */
...
class HRS_Custom_Post_Type {
    ...
    public function register_post_type() {
        ...
        $args = array(
            ...
            'show_ui'             => true,
            'show_in_menu'        => '5d-review-builder', // 5D Review Builderのサブメニューに追加
            'query_var'           => true,
    ...
    public function render_hotel_info_metabox($post) {
        ...
        $hotel_address = get_post_meta($post->ID, '_hrs_hotel_name', true);
    ...
    /**
     * HQCスコアメタボックスを表示 ✅ 修正版（H/Q/C個別入力対応）
     */
    public function render_hqc_info_metabox($post) {
        wp_nonce_field('hrs_hqc_info_nonce', 'hrs_hqc_info_nonce_field');

        // ✅ 新キーで取得（旧キーからのマイグレーション対応）
        $h_score = floatval(get_post_meta($post->ID, '_hrs_h_score', true));
        $q_score = floatval(get_post_meta($post->ID, '_hrs_q_score', true));
        $c_score = floatval(get_post_meta($post->ID, '_hrs_c_score', true));
        $total_score = floatval(get_post_meta($post->ID, '_hrs_hqc_score', true));

        // 旧キーから一時移行（保存時に新キーへ永続化）
        if (!$h_score && !$q_score && !$c_score) {
            $h_score = floatval(get_post_meta($post->ID, '_hrs_hqc_h_score', true));
            $q_score = floatval(get_post_meta($post->ID, '_hrs_hqc_q_score', true));
            $c_score = floatval(get_post_meta($post->ID, '_hrs_hqc_c_score', true));
        }

        if (!$total_score && ($h_score || $q_score || $c_score)) {
            $total_score = $this->calculate_hqc_score($h_score, $q_score, $c_score);
        }

        $label = $this->get_hqc_label($total_score);
        $label_colors = array(
            'excellent' => '#22c55e',
            'good'      => '#3b82f6',
            'fair'      => '#f59e0b',
        ...
        ?>
        <div class="hqc-total-display" style="text-align: center; padding: 10px; margin-bottom: 15px;">
            <?php if ($total_score): ?>
                <div style="font-size: 32px; font-weight: bold; color: <?php echo esc_attr($color); ?>;">
                    <?php echo esc_html(number_format($total_score * 100, 1)); ?>
                </div>
        ...
        <table class="form-table">
            <tr>
                <th><label for="hrs_h_score">H: Hotel<br><small>宿の質（設備・清潔感など）</small></label></th>
                <td>
                    <input type="range" id="hrs_h_score" name="hrs_h_score" 
                           min="0" max="1" step="0.01"
                           value="<?php echo esc_attr($h_score); ?>"
                           onchange="this.nextElementSibling.textContent = (this.value * 100).toFixed(0) + '%'">
                    <span><?php echo esc_html(($h_score * 100).toFixed(0)); ?>%</span>
                </td>
            </tr>
            <tr>
                <th><label for="hrs_q_score">Q: Quality<br><small>体験の質（五感・非日常感）</small></label></th>
                <td>
                    <input type="range" id="hrs_q_score" name="hrs_q_score" 
                           min="0" max="1" step="0.01"
                           value="<?php echo esc_attr($q_score); ?>"
                           onchange="this.nextElementSibling.textContent = (this.value * 100).toFixed(0) + '%'">
                    <span><?php echo esc_html(($q_score * 100).toFixed(0)); ?>%</span>
                </td>
            </tr>
            <tr>
                <th><label for="hrs_c_score">C: Content<br><small>コンテンツの質（記事構成・情報量）</small></label></th>
                <td>
                    <input type="range" id="hrs_c_score" name="hrs_c_score" 
                           min="0" max="1" step="0.01"
                           value="<?php echo esc_attr($c_score); ?>"
                           onchange="this.nextElementSibling.textContent = (this.value * 100).toFixed(0) + '%'">
                    <span><?php echo esc_html(($c_score * 100).toFixed(0)); ?>%</span>
                </td>
            </tr>
        </table>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            function updateTotal() {
                const h = parseFloat(document.getElementById('hrs_h_score').value) || 0;
                const q = parseFloat(document.getElementById('hrs_q_score').value) || 0;
                const c = parseFloat(document.getElementById('hrs_c_score').value) || 0;
                const total = h * 0.4 + q * 0.4 + c * 0.2;
                
                const label = total >= 0.85 ? 'Excellent' : 
                             total >= 0.70 ? 'Good' :
                             total >= 0.50 ? 'Fair' : 'Poor';
                const color = total >= 0.85 ? '#22c55e' : 
                             total >= 0.70 ? '#3b82f6' :
                             total >= 0.50 ? '#f59e0b' : '#ef4444';
                
                const display = document.querySelector('.hqc-total-display');
                display.innerHTML = 
                    '<div style="font-size:32px;font-weight:bold;color:' + color + '">' + 
                    (total * 100).toFixed(1) + 
                    '</div><div style="font-size:14px;color:' + color + ';text-transform:uppercase">' + 
                    label + '</div>';
            }
            
            ['hrs_h_score', 'hrs_q_score', 'hrs_c_score'].forEach(id => {
     ...
     * HQC総合スコア計算 ✅ 新規追加
     */
    private function calculate_hqc_score($h, $q, $c) {
        return max(0, min(1, $h * 0.4 + $q * 0.4 + $c * 0.2));
    }
    ...
    public function save_meta_boxes($post_id, $post) {
        ...
        // HQCスコア nonce 検証
        if (isset($_POST['hrs_hqc_info_nonce_field']) && 
            !wp_verify_nonce($_POST['hrs_hqc_info_nonce_field'], 'hrs_hqc_info_nonce')) {
            return;
        }
        ...
        // ✅ HQCスコア保存（新キー）
        if (isset($_POST['hrs_h_score']) || isset($_POST['hrs_q_score']) || isset($_POST['hrs_c_score'])) {
            $h = isset($_POST['hrs_h_score']) ? floatval($_POST['hrs_h_score']) : 0;
            $q = isset($_POST['hrs_q_score']) ? floatval($_POST['hrs_q_score']) : 0;
            $c = isset($_POST['hrs_c_score']) ? floatval($_POST['hrs_c_score']) : 0;

            // 新キーで保存
            update_post_meta($post_id, '_hrs_h_score', $h);
            update_post_meta($post_id, '_hrs_q_score', $q);
            update_post_meta($post_id, '_hrs_c_score', $c);

            // 総合スコア再計算・保存
            $total = $this->calculate_hqc_score($h, $q, $c);
            update_post_meta($post_id, '_hrs_hqc_score', $total);

            // 古いキー削除（クリーンアップ）
            delete_post_meta($post_id, '_hrs_hqc_h_score');
            delete_post_meta($post_id, '_hrs_hqc_q_score');
            delete_post_meta($post_id, '_hrs_hqc_c_score');
        }
    }

    /**
     * 管理画面カラムを追加 ✅ 修正版（H/Q/C/HQC 4カラム）
     */
    ...
    public function render_admin_columns($column, $post_id) {
        switch ($column) {
            ...
            case 'h_score':
                $score = floatval(get_post_meta($post_id, '_hrs_h_score', true));
                echo $score ? esc_html(number_format($score * 100, 0)) . '%' : '—';
                break;

            case 'q_score':
                $score = floatval(get_post_meta($post_id, '_hrs_q_score', true));
                echo $score ? esc_html(number_format($score * 100, 0)) . '%' : '—';
                break;

            case 'c_score':
                $score = floatval(get_post_meta($post_id, '_hrs_c_score', true));
                echo $score ? esc_html(number_format($score * 100, 0)) . '%' : '—';
                break;
            ...
            case 'hqc_score':
                ...
                if ($score) {
                    $label = $this->get_hqc_label($score);
                    $colors = array(
                        'excellent' => '#22c55e',
                        'good'      => '#3b82f6',
                        'fair'      => '#f59e0b',
                    ...
                    echo esc_html(number_format($score * 100, 1));
                    echo '</span>';
                } else {
                    echo '—';
                }
