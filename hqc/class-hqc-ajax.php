 ...
 * 
 * HQC Generator の非同期処理（保存・生成・キュー・プレビュー・自動最適化等）を管理
 * 
 ...
 * 
 * 変更履歴:
 * - 7.1.0: 自動最適化 ajax_auto_optimize() 追加
...
class HRS_Hqc_Ajax {
     ...
     * AJAXハンドラーを登録
     * 
     * ★注意: メインのAJAX登録は HRS_Ajax_Registry が行う。
     * ここでは Registry未登録のアクションのみ補完登録する。
     */
    public static function init() {
        // Registry未登録のアクションのみ補完
        $supplementary = [
            'hrs_hqc_autosuggest'   => 'ajax_autosuggest',
            'hrs_add_to_queue'      => 'ajax_add_to_queue',
            'hrs_remove_from_queue' => 'ajax_remove_from_queue',
            'hrs_process_queue'     => 'ajax_process_queue',
            'hrs_hqc_auto_optimize' => 'ajax_auto_optimize',
        ];

        foreach ($supplementary as $action => $method) {
    ...
    // ========================================
    // ★ Ajax Registry 互換エイリアス
    // Registry は ajax_ プレフィックスなしのメソッド名で呼び出す
    // ========================================

    /** @see ajax_save_settings() */
    public static function save_settings() {
        self::ajax_save_settings();
    }

    /** @see ajax_apply_preset() */
    public static function apply_preset() {
        self::ajax_apply_preset();
    }
    ...
    /** @see ajax_auto_optimize() */
    public static function auto_optimize() {
        self::ajax_auto_optimize();
    }
     ...
     * ペルソナは設定全体の一部として保存する
     */
    public static function save_persona() {
        check_ajax_referer('hrs_hqc_nonce', 'nonce');
        if (!current_user_can('edit_posts')) {
            wp_send_json_error(['message' => __('権限がありません。', '5d-review-builder')], 403);
        }

        $persona = isset($_POST['persona']) ? sanitize_key(wp_unslash($_POST['persona'])) : '';
        if (empty($persona)) {
            wp_send_json_error(['message' => __('ペルソナが指定されていません。', '5d-review-builder')], 400);
        }

        $settings = get_option('hrs_hqc_settings', []);
        ...
        $settings['h']['persona'] = $persona;
        update_option('hrs_hqc_settings', $settings);

        wp_send_json_success([
            'message' => sprintf(__('ペルソナ「%s」を保存しました。', '5d-review-builder'), $persona),
            'persona' => $persona,
        ]);
    }
    ...
    /**
     * ★【v7.1.0】再分析結果からHQCパラメータを自動最適化
     * 
     * 記事IDを受け取り、再分析 → weak_points → パラメータ自動調整を実行
     * 調整後のパラメータは記事メタに保存され、再生成やキュー追加に使用可能
     */
    public static function ajax_auto_optimize() {
        check_ajax_referer('hrs_hqc_nonce', 'nonce');
        if (!current_user_can('edit_posts')) {
            wp_send_json_error(['message' => __('権限がありません。', '5d-review-builder')], 403);
        }
        ...
        if (empty($post_id)) {
            wp_send_json_error(['message' => __('記事IDが指定されていません。', '5d-review-builder')], 400);
        }
        ...
        // Auto Optimizer の読み込み
        self::ensure_auto_optimizer();

        if (!class_exists('HRS_HQC_Auto_Optimizer')) {
            wp_send_json_error([
                'message' => __('HQC Auto Optimizer クラスが見つかりません。', '5d-review-builder')
            ], 500);
        }

        $optimizer = HRS_HQC_Auto_Optimizer::get_instance();
        $result = $optimizer->optimize_from_post($post_id);

        if (is_wp_error($result)) {
            wp_send_json_error(['message' => $result->get_error_message()], 500);
        }

        $summary = HRS_HQC_Auto_Optimizer::format_optimization_summary($result);

        wp_send_json_success([
            'message'        => $result['change_count'] > 0
                ? sprintf(__('%d件のパラメータを自動調整しました。', '5d-review-builder'), $result['change_count'])
                : __('最適化の必要はありません（現在のパラメータで十分です）。', '5d-review-builder'),
    ...
    /**
     * HRS_HQC_Auto_Optimizer の読み込みを保証
     * 
     * @since 7.1.0
     */
    private static function ensure_auto_optimizer() {
        if (class_exists('HRS_HQC_Auto_Optimizer')) {
            return;
        }

        $paths = [
            plugin_dir_path(dirname(dirname(__FILE__))) . 'learning/class-hqc-auto-optimizer.php',
            dirname(dirname(dirname(__FILE__))) . '/learning/class-hqc-auto-optimizer.php',
        ];
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('[HRS] WARNING: HRS_HQC_Auto_Optimizer not found in any expected path');
        }
    }
